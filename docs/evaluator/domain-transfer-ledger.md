...
- interpretation: the exported candidate set could compare source-worktree baseline and BG-selected branches over the same examples.
...
- interpretation: on exported code candidates, BG did not outperform the source-worktree final selection, random expectation, or a simple stage heuristic.
