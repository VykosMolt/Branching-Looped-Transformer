...
Current runtime note, 2026-05-13: local GGUF reviewer models and the old multi-model reviewer modes have been purged. Ouro now routes through the primary `ouro-rltt-local` transformers backend, with direct, agent, auto, and classify modes only. Older Qwen/DeepSeek reviewer notes below are historical and no longer describe active code paths.
...
## 2026-05-09 continuation: generic route-prior control, syntax honesty, and current hard boundary

- Added generic route scaffold fields in `/home/moloch/local_agent/ouro_route_priors.py`:
  - `generic_route_scaffold_fields(...)`;
  - fills concrete route-sketch fields from active priors only, not benchmark task names.
- Wired the generic scaffold into `/home/moloch/local_agent/ouro_agent_improved.py`:
  - deterministic route-frontier scaffold now emits parseable, route-gate-ready fields;
  - active route-prior tasks use a generic-prior scaffold fastpath instead of spending model calls on malformed route sketches.
- Tightened wrapper execution control:
  - code-fence sanitation now extracts embedded and malformed inline fenced code blocks;
  - external evaluator runtime exceptions such as `NameError`/`IndexError` are treated as implementation repairs, not automatic route-invariant failures;
  - undefined capitalized helper/class calls are caught before execution;
  - syntax-failed code is retained in working memory for repair context but is not surfaced as an answer;
  - unverified syntax/incomplete/helper-failed code now returns no-code honest failure.
- Runtime stability changes:
  - hard-code wallclock overrides now reach the runtime guard correctly;
  - HF generation receives a per-call `max_time`;
  - CUDA temporary tensors are released after generation;
  - high-risk route-prior tasks skip multi-candidate sampling and use a larger one-pass code budget.
- Focused wrapper tests:
  - `/home/moloch/ouro_project/tests/unit/test_local_agent_wrapper.py`;
  - latest result: `135 passed`.
- Current hard route-ladder results, offline/no expert/no oracle:
  - dynamic output:
    - `/home/moloch/ouro_project/runs/local_agent_route_ladder/route_ladder_scaffold_dynamic_syntax_memory_latest.json`;
    - result: `honest_abstain`, `360.359s`;
    - failure mode: Ouro repeatedly generated substantial but syntactically incomplete Python and never reached a verified evaluator pass.
  - minimum-XOR output:
    - `/home/moloch/ouro_project/runs/local_agent_route_ladder/route_ladder_scaffold_minxor_current_latest.json`;
    - result: `honest_abstain`, `213.43s`;
    - failure mode: repeated route family rejected (`route_reuse_rejected`) instead of surfaced.
- Current normal/hard DSA regression:
  - output:
    - `/home/moloch/ouro_project/runs/local_agent_dsa_coding/dsa_current_normal_hard_latest.json`;
  - mode: direct, no oracle/reference fastpath;
  - all four passed:
    - `subarray_sum_count`: `9.009s`;
    - `longest_increasing_subsequence`: `5.442s`;
    - `word_ladder_length`: `8.946s`;
    - `count_smaller_after_self`: `5.021s`.
- Current interpretation:
  - wrapper honesty/control is improved: no internet leakage, no code surfacing for hazardous hard attempts, bounded runtime, and standard DSA remains intact;
  - the remaining hard-task blocker is not route-card detection anymore;
  - Ouro still fails to finish or diversify correct implementations for the two adversarial route-prior tasks under the no-oracle/no-expert setting.

## 2026-05-09 audit pass + post-audit harness results

Audit (read-only) over the local-agent module set, then targeted fixes applied across six batches in `/home/moloch/local_agent/`. Tests `pytest tests/unit/test_local_agent_wrapper.py` (135 passed) and the wrapper+self-model+self-model-integration set (187 passed) pass after every batch.

Fixes applied:

- Oracle leakage closed at the source:
  - `_recognized_python_reference_solution` and `_verify_direct_ml_theory` in `ouro_oracles.py` now gate on `ORACLE_LOOKUPS_ENABLED` (the latter was previously ungated, so canonical ML-theory answers could substitute even when policy was off);
  - `ouro_direct.py` now stamps `from_lookup: true; source: local_oracle; kind: …` on hidden-test-pass and ML-theory rewrites, not only on reference fallback.
- Server hardening in `ouro_server.py`:
  - bearer auth uses `secrets.compare_digest`, scheme match is case-insensitive;
  - empty `Host` header is rejected, IPv6 zone-id stripped;
  - body cap enforced from actual bytes read, chunked transfer rejected, malformed `Content-Length` rejected;
  - 500 handler logs the traceback server-side and returns only `{ok:false, error:"internal server error", request_id}`;
  - `LOCAL_AGENT_SERVER_TOKEN` rejected if it contains characters outside `[A-Za-z0-9_-]` so the embedded UI JS string literal cannot be broken out of.
- Runtime correctness:
  - `ouro_runtime_guard.py`: replaced the `started_at > 1e8` magnitude heuristic with an explicit `started_at_kind="wall"|"monotonic"` parameter (the heuristic could silently produce instant-negative deadlines on hosts with monotonic time greater than 1e8); `adjust_generation_for_runtime_budget` now returns `(0, …)` past deadline instead of granting a 64-token floor;
  - `ouro_backend.py`: `max_time` is only applied when `> 0` (an explicit no-limit no longer clamps to 250 ms); `gen.OutOfMemoryError` retry now `gc.collect()`s and clears `exit_records` before the half-budget retry; the exit-telemetry forward hook is bounded at 256 records to stop unbounded per-token growth;
  - `ouro_model_managers.py`: latent-loop telemetry no longer `setdefault`s on through `LOCAL_AGENT_OURO_CAPTURE_LOOP_TELEMETRY=0`.
- Memory and retrieval:
  - `ouro_memory.py`: query terms removed from the IDF corpus (the prior code deflated weights on the rare task terms that should have driven retrieval); polarity classifier broadened to recognize `fail`/`error`/`block`/`timeout`/`mismatch`/`abort`/`exhaust`/`rejected`/`abstain`/`hazard` instead of a small whitelist; `record()` now flushes, busts the in-process terms+weights cache for the path, and surfaces write errors;
  - `ouro_route_priors.py`: tightened `_has_graph_pair_query_signals` so the regex no longer matches the bare English words `\bu\b`, `\bv\b`, `\bfrom\b`, `\bto\b`.
- Self-model coherence:
  - `ouro_self_model.py`: persisted `decision.route` on episodes so the `direct_negative` gate can see `self_verify` outcomes; `_policy_calibration_stats` now picks the bucket with the most evidence (a single-shot exact bucket no longer shadows a 30-sample broad bucket); broke aliasing between `last_self_predictor_prediction` and the last entry of `recent_self_predictor_predictions`;
  - `ouro_self_store.py`: atomic `_save` via `tmp` + `os.replace`; corrupted JSON on load is quarantined to `*.corrupt-{ts}` instead of silently wiping history;
  - `ouro_self_predictor.py`: removed the double smoothing (Beta(1,1) on top of confidence blend) so predicted failure can actually trip with realistic evidence; trusts the learned rate directly past `OURO_SELF_PREDICTOR_MIN_EVIDENCE`; confidence denominator 5.0 → 4.0.
- I/O surface:
  - `ouro_external_experts.py`: CDP stable-time uses `time.monotonic`, requires two consecutive idle reads when `required_stable_sec=0`, and the previous boundary that doubled the wait window is fixed; tab is closed only on success so failed/aborted streams stay inspectable;
  - `ouro_tooling.py`: `write_file` re-validates the resolved parent (and the deepest existing ancestor when the parent doesn't exist) against `LOCAL_AGENT_WRITABLE_ROOT.resolve()` before any `mkdir`, so pre-planted symlinks in the writable root cannot redirect directory creation;
  - `ouro_webtools.py`: reader URL no longer double-schemes `https://r.jina.ai/http://https://example.com`, refuses URLs containing userinfo (no credential leakage to the reader); composite provider chain only retries when the result is exactly `"No results."`, not when the substring appears anywhere; Exa now sends only `x-api-key` (the redundant `Authorization: Bearer` header was dropped).

Post-audit harness runs under `bwrap --unshare-net` (no internet, GPU available). Search/oracles/research/experts disabled.

- Normal DSA harness:
  - output: `/home/moloch/ouro_project/runs/local_agent_dsa_coding/post_audit_baseline.json`;
  - command: `--ut-steps 2 --max-tokens 512 --react-steps 3 --repair-rounds 0`;
  - results, all `outcome_class=solved`, all `has_lookup=false`:
    - `subarray_sum_count`: passed, `8.82s`;
    - `longest_increasing_subsequence`: passed, `5.47s`;
    - `word_ladder_length`: passed, `8.98s`;
    - `count_smaller_after_self`: passed, `5.12s`;
    - total: `28.4s`.
- Devils harness (post-audit):
  - output: `/home/moloch/ouro_project/runs/local_agent_hard_coding/post_audit_devils.json`;
  - command: `--full-capability --ut-steps 2 --max-tokens 768 --react-steps 3 --repair-rounds 2 --task-wallclock-sec 360`;
  - `offline_dynamic_connectivity`: failed in `211.95s`, `outcome_class=honest_abstain`, `has_lookup=false`, `has_code=false`, `failure_kind=agent_max_steps`, `err="no code block returned"`;
  - `minimum_xor_paths`: failed in `210.21s`, `outcome_class=honest_abstain`, `has_lookup=false`, `has_code=false`, `failure_kind=agent_max_steps`, `err="no code block returned"`;
  - `outcome_counts`: `{"honest_abstain": 2}`;
  - GPU returned to idle (~561 MiB) after both tasks; no thermal pause was needed.

Interpretation:

- the audit fixes are honesty/control improvements: oracle paths now fail closed by default, retrieval weights actually weight rare terms, runtime deadlines are robust to long-uptime hosts, the predictor can actually trip with realistic evidence, and the self-model store no longer silently wipes itself on a partial write;
- the devils harness still produces `honest_abstain` on both tasks, matching the prior frontier-controller runs;
- the residual blocker remains route-to-code execution: even when a full route is supplied, RLTT generates wrong or syntactically broken implementations, and the wrapper now correctly refuses to surface them.

## Potential future work — route-to-code-execution improvement

The post-audit devils harness confirms again that the wrapper is no longer the limiting factor on `offline_dynamic_connectivity` and `minimum_xor_paths`. Cracking these tasks would need an improvement that moves below the wrapper layer or alongside it:

- model-level work:
  - additional retraining or fine-tuning targeted at adversarial route-to-code execution (segment-tree-over-time + rollback DSU; cycle-XOR linear basis; algebraic-walk decomposition);
  - decoding-time changes such as constrained decoding for Python AST validity, contrastive decoding against a smaller checker model, or schema-conditioned tool-call decoding so the model cannot emit syntactically broken `[Action]: python` payloads;
  - a small specialized "draft" model that proposes brute-force/checker code first, with RLTT acting as the verifier/optimizer;
- wrapper-level companion (still benchmark-agnostic, but bigger than an audit fix and needs proper test coverage before shipping):
  - a generic small-graph brute-force-first curriculum: when route priors are active and an external evaluator is available, force the first hard-code attempt to emit a brute-force `brute_force_{name}(args)` for small inputs together with `assert brute_force(x) == optimized(x)` over a randomized small set, and only after that differential check fires can the optimized version be surfaced;
  - this is on the existing future-work list ("a curriculum/probe phase that asks the model to infer invariants from tiny executable cases, then distills the invariant into code") and was deliberately not shipped in this audit pass because the additional control flow would need its own focused tests and a route-prior-only gate to avoid regressing the normal DSA path;
  - if attempted, it should:
    - reuse the existing `EVALUATOR-BACKED FRONTIER ATTEMPT` compact-code path;
    - require both a brute-force entrypoint and an optimized entrypoint in the same tool call, with the brute force used as the independent checker proxy;
    - only activate when route priors fire and an external evaluator is active, so ordinary DSA stays on the direct path;
    - remain task-name-agnostic (no benchmark hardcoding); cards and priors must do all the steering.

## 2026-05-14 repo layout output-placement update

After the `tests/` and `tools/` move into `utilities/`, active local-agent defaults were aligned with the new tree:

- `LOCAL_AGENT_WRITABLE_ROOT` now defaults to `/home/moloch/ouro_project/artifacts/local_agent` instead of the source tree.
- The existing generated `src/local_agent/projects/` and `src/local_agent/browser_profiles/` directories were moved to `artifacts/local_agent/projects/` and `artifacts/local_agent/browser_profiles/`.
- Local-agent harness outputs now default to `artifacts/reports/local_agent/`:
  - DSA harness: `artifacts/reports/local_agent/dsa_coding/`;
  - hard-code harness: `artifacts/reports/local_agent/hard_coding/`;
  - route ladder: `artifacts/reports/local_agent/route_ladder/`;
  - self-model probe: `artifacts/reports/local_agent/self_model_probe_*`.
- `launch_ouro_ui.sh` now exports `LOCAL_AGENT_WRITABLE_ROOT` to the artifacts path and places external-expert browser profiles under that root.
- The root `sitecustomize.py` hook was removed; the project venv path file now carries both `src/` and `src/local_agent/`, and pytest carries the same roots through `pytest.ini`.

Historical command examples above may still mention old `runs/` paths. Current wrappers and maps are the source of truth for new runs.
