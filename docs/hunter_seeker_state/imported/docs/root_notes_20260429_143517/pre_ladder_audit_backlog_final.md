<!-- Imported from `docs/root_notes_20260429_143517/pre_ladder_audit_backlog_final.md` during Hunter-Seeker state consolidation on 2026-05-14. -->
<!-- Original SHA256: 5b3e2dda503bf5f76a8fef2a02f2028841ae6028e3673b43ba22c4bacac60d9a; original line count: 914. -->
...
## Quick-ladder anchor issue: frozen evaluator GRU + cuDNN backward

Discovered after the quick ladder completed structurally through Steps 0–7.

### Symptom

The ladder now runs through all steps and writes `measurement_run_1.json` for each step, but anchor-training steps emit:

```text
[Anchor-Train] step failed: RuntimeError('cudnn RNN backward can only be called in training mode') — skipping.
```

This appeared in anchor-enabled steps such as:

```text
Step 3.5 — Ouro + anchor only
Step 7   — full system
```
...
```text
- anchor attempts are firing
- encoder unfreeze is now happening
- the ladder no longer crashes
```
...
- anchor success counters will not represent real anchor training until fixed
```

### Cause

`FrozenCLTAnchor` freezes the evaluator and calls:
...
This is conceptually correct: the evaluator is an anchor, not a learnable module.

However, the evaluator contains a GRU. Anchor loss needs gradients to flow **through** the frozen evaluator into its inputs, then back into encoder/Ouro input tokens. With cuDNN RNNs, backward through an RNN in eval mode can trigger:

```text
cudnn RNN backward can only be called in training mode
```

So the issue is not “the evaluator should be trained.” It is:

```text
frozen eval-mode GRU + need input-gradient backward + cuDNN restriction
```

The relevant call site is in `claude_sandbox/anchor_loss.py`:

```text
grep -n "self.evaluator\|preference\|forward" claude_sandbox/anchor_loss.py

70: self.evaluator = PairwiseEvaluator()
83: self.evaluator.eval()
84: for p in self.evaluator.parameters():
108: Return raw pairwise preference logit
119: return self.evaluator(...)
131: -log_sigmoid(score)
```

### Preferred permanent fix

Keep evaluator weights frozen, but disable cuDNN around the frozen evaluator forward used for anchor scoring.

Patch shape inside the method that currently does:
...
```text
- evaluator parameters remain frozen
- gradients still flow through evaluator inputs into encoder/Ouro tokens
- avoids cuDNN's eval-mode RNN backward restriction
- avoids switching evaluator to train mode
- avoids activating dropout in the evaluator
```

### Acceptable fallback, but less clean

Set the evaluator module to train mode while keeping parameters frozen:
...
But this is less clean because the evaluator architecture has dropout. Train mode would activate dropout and make anchor scores noisy unless dropout is disabled separately.

So the cuDNN-disabled local forward is better.

### What to verify after patch

Rerun quick ladder or at least an anchor-enabled quick step and check:

```text
- no "cudnn RNN backward can only be called in training mode"
- anchor.attempts > 0
- anchor.successes > 0
- skipped_oom / skipped_frozen_encoder / skipped_no_evaluator not dominating
- current_batch_size appears
- loss_ema appears if successes > 0
```
...
Status: permanent bugfix.

Reason:

```text
The evaluator should stay frozen, but anchor loss intentionally requires gradients through the frozen evaluator into its inputs.
Disabling cuDNN for that small frozen-GRU forward is a targeted compatibility fix, not a ladder-only hack.
```

Add this to the permanent cleanup checklist:

```text
6. Wrap frozen CLT evaluator anchor forward in torch.backends.cudnn.flags(enabled=False).
```
